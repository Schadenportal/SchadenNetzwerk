...
export default function ChatRoomGroup({ participants }: Props) {
  ...
  const handleOpen = useCallback((participant: UserModel) => {
    ...
    <Scrollbar sx={{ height: 56 * 4 }}>
      {participants.map((participant) => (
        <ListItemButton key={participant.userId} onClick={() => handleOpen(participant)}>
          ...
          >
            <Avatar alt="avatar" src={participant.photoURL} />
          </Badge>

          <ListItemText
            sx={{ ml: 2 }}
            primary={participant.fullName || `${participant.firstName} ${participant.lastName}`}
