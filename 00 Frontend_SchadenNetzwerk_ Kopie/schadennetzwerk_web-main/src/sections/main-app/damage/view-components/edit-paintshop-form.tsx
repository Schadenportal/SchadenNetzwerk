...
import { updateDamageInfo } from 'src/services/firebase/functions';
...
type Props = {
  damageInfo: DamageModel;
  workshopInfo?: WorkshopModel;
  paintProvider?: ServiceProviderModel;
}

export default function EditPaintShopForm({
  damageInfo,
  workshopInfo,
  paintProvider
}: Props) {
  ...
  const [previewUrl, setPreviewUrl] = useState("N/A");
  ...
  const defaultValues = useMemo(
    () => ({
      parties: {
        client: {
          company: '',
          contactPerson: workshopInfo?.name || '',
          phone: workshopInfo?.phone || '',
          email: workshopInfo?.email || '',
          address: `${workshopInfo?.street || ''}, ${workshopInfo?.postalCode || ''} ${workshopInfo?.city || ''}`
        },
        contractor: {
          company: '',
          contactPerson: paintProvider?.name || '',
          phone: paintProvider?.phone || '',
          email: paintProvider?.email || '',
          address: `${paintProvider?.street || ''}, ${paintProvider?.postalCode || ''} ${paintProvider?.city || ''}`
        }
      },
      vehicle: {
        makeModel: damageInfo.customerVehicleModel || '',
        vin: damageInfo.customerVehicleVINNumber || '',
        licensePlate: damageInfo.customerVehicleLicensePlate || '',
        paintCode: '',
        mileageKm: null
      },
      scope: {
    ...
    [paintProvider, workshopInfo, damageInfo]
  );
  ...
  // Reset form when damageInfo changes
  useEffect(() => {
    reset(defaultValues);
  }, [damageInfo, reset, defaultValues]);
  ...
  const handleSave = async () => {
    ...
    if (fileResponse.ok) {
      ...
      const file = new File([fileBlob], "paintshop.pdf", { type: "application/pdf" });
      const filePath = `paint_shops/${format(new Date(), 'yyyyMMddHHmmss')}.pdf`;
      const downloadUrl = await uploadFile(file, filePath);
      if (!downloadUrl) {
        setIsSaving(false);
        enqueueSnackbar(t('failed_to_upload_pdf'), { variant: 'error' });
        return;
      }
      // create data and updateDamageInfo function
      const data = {
        damageId: damageInfo.damageId,
        addPaintShopPdf: true,
        paintShopPdfUrl: downloadUrl,
      };
      try {
      ...
      setIsSaving(false);
      enqueueSnackbar(t('failed_to_save_pdf'), { variant: 'error' });
    }
  };
  ...
  return (
    <Card>
      <Stack direction="row" alignItems="center" spacing={1} p={3}>
        <Box sx={{ flexGrow: 1 }} />
        <Button
          color="inherit"
        ...
        >
          {activeStep === steps.length - 1 ? t('generate_pdf') : t('next')}
        </Button>
      </Stack>
                  ...
                  <>
                    ...
                    <Box sx={{ mt: 2 }}>
                      <LoadingButton
                        ...
                        disabled={isSubmitting || isSaving || previewUrl === 'N/A'}
                        onClick={handleSave}
                        sx={{ m: 3, float: 'right' }}
                      >
                        {t('save')}
