...
import { removeCostEstimation } from 'src/services/firebase/functions';
...
type Props = {
  item: CostEstimateModel,
  onDeleted: (id: string) => void;
  onEditing: (id: string) => void;
}

export default function CostEstimationListComp({ item, onDeleted, onEditing }: Props) {
  ...
  const onEdit = () => {
    onEditing(item.costEstimationId);
  }

  const onDelete = async () => {
    setIsDeleting(true);
    try {
      const res: any = await removeCostEstimation({ costEstimationId: item.costEstimationId });
      setIsDeleting(false);
      ...
      onDeleted(item.costEstimationId);
    } catch (error) {
      setIsDeleting(false);
  ...
  return (
    <>
      <Card sx={{ p: 3, mt: 3 }}>
        ...
        <Typography variant='subtitle1' color="warning.main">{t('cost_estimated_at')} {format(item.createdAt.toDate(), 'dd MMM yyyy')}</Typography>
