...
export const getFileFormatFromUrl = (fileUrl: string | undefined) => {
  if (!fileUrl) { return '' }
  const match = fileUrl.match(/\.([a-zA-Z0-9]+)\?/);
  const fFormat = match ? match[1] : '';
  return fFormat;
}
...
export function fileFormat(fileUrl: string | undefined) {
  ...
  switch (fileUrl?.includes(fileTypeByUrl(fileUrl))) {
    ...
    default: {
      const myFormat = getFileFormatFromUrl(fileUrl);
      if (myFormat) {
        if (FORMAT_IMG.includes(myFormat)) {
          format = 'image';
        } else {
          format = myFormat;
        }
